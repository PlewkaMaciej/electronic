...
export const defaultRTVFilters: Filters = {
  brand: [],
  type: [],
  resolution: [],
  state: [],
...
export function useRTVFilters() {
  const [filters, setFilters] = useState<Filters>(defaultRTVFilters);
...
export function filterRTVOffers(
  offers: RTVOffer[],
  filters: Filters,
...
export default function RTVFilters({
  filters,
  handleMultiSelect,
  setFilters,
}: Props) {
  return (
    <>
      <FilterSection
        title="Marka"
        options={["Samsung", "LG", "Sony", "Panasonic", "Philips"]}
        selected={filters.brand}
        onToggle={(value) => handleMultiSelect("brand", value)}
      />
      <FilterSection
        title="Typ"
        options={["Telewizor", "Odtwarzacz", "Amplituner", "Projektor"]}
